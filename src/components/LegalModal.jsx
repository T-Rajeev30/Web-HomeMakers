...
export default function LegalModal({ title, onClose, children, scrollToId }) {
  const bodyRef = useRef(null);

  useEffect(() => {
    if (!scrollToId) return;
    // Wait a tick for content to be in the DOM, then scroll within the
    // modal's own scroll container (not the page — this is a fixed overlay).
    const t = setTimeout(() => {
      const el = bodyRef.current?.querySelector(`#${scrollToId}`);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-surface w-full sm:max-w-2xl max-h-[88vh] rounded-t-2xl sm:rounded-2xl flex flex-col shadow-card">
        ...
        <div ref={bodyRef} className="px-5 py-4 overflow-y-auto">
          {children}
        </div>
