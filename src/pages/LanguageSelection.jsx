    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-grow w-full max-w-lg mx-auto px-margin-mobile py-stack-lg flex flex-col">
        <div className="mb-stack-lg">
          ...
          <p className="font-headline-md text-headline-md text-primary opacity-90">अपनी भाषा चुनें</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-stack-lg flex-grow">
          {languages.map((lang) => {
            ...
            return (
              ...
              >
                <span className={`text-headline-lg font-headline-lg mb-2 ${active ? "text-primary" : ""}`}>
                  {lang.glyph}
                </span>
                <span className="font-label-lg text-label-lg text-on-surface">{lang.name}</span>
              </button>
            );
          })}
        </div>

        <Button full icon="arrow_forward" onClick={() => navigate("/signup")}>
          Continue
        </Button>
      </main>
    </div>
